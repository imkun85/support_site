...
[flaskfarm/support_site](https://github.com/flaskfarm/support_site)를 포크해서 버그를 고친 버전이다.

#### 패치 내역

**1. `site_watcha.py` — 왓챠 API hang 방지 (`timeout=30`)**
...
**2. `site_watcha.py` — `_search_api()` None 체크 추가**

`get_json()` 응답이 `None`이거나 `result` 키가 없을 때 `NoneType` 에러가 나는 것을 방지했다.

#### 원본 동기화

upstream(`flaskfarm/support_site`)의 업데이트를 주기적으로 머지한다.
`site_watcha.py`의 위 패치는 머지 후에도 유지해야 한다(원본엔 반영되지 않음).
